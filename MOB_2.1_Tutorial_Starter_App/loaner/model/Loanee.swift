...
public class Loanee: NSObject, NSCoding {
    ...
    // NSCoding protocol requires having Keys for our variables
    enum Keys: String {
        case name = "name"
    ...
    // NSCoding protocol requires an encode method toencode our variables based on the Key
    public func encode(with aCoder: NSCoder) {
        aCoder.encode(name, forKey: "name")
        aCoder.encode(contactNumber, forKey: "contactNumber")
    }
    
    // NSCoding protocol requires an init method which acts asa decoder for our variables based on the Key
